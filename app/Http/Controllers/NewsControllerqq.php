	  ...
	  */
	  public function postUpload() {
		...
		if ($file->isValid()) {
          ...
          $oss = AliyunOSS::boot(config('Service.aliyun.oss'),  config('Service.aliyun.AccessKeyId'), config('Service.aliyun.AccessKeySecret'));
          // 设置 Bucket
          $oss = $oss->setBucket(config('Service.aliyun.Bucket'));
          // 上传一个文件（示例文件为 public 目录下的 robots.txt）
          // 两个参数：资源名称、文件路径
          $oss->uploadFile($file_name, public_path($uploadDir . $file_name));
        // 从服务器获取这个资源的 URL 并打印
        // 两个参数：资源名称、过期时间

          echo $oss->getUrl($file_name,"86400");
          die;
